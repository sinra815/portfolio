...
// 진단용: 캐시를 무시하고 토큰을 강제로 새로 발급해보고, 성공 여부와 NH가 돌려준 메시지만
// (토큰 값 자체는 절대 포함하지 않고) 알려준다 — 지금 설정된 앱키/시크릿이 유효한지 확인용.
// 발급에 성공하면 그 토큰을 캐시에도 반영한다 — 앱키/시크릿이 막 바뀐 직후라 예전 앱키로
// 발급된 캐시가 아직 남아있는 경우, 뒤이은 실제 조회 호출이 그 무효 토큰을 계속 재사용하는
// 문제를 막기 위함.
export async function checkNhToken(redis, account){
  const mask = (s) => (s ? `${s.slice(0, 4)}...${s.slice(-4)} (길이 ${s.length})` : '(미설정)');
  try {
    const { token, expiresAt } = await issueNhToken(account);
    await redis.set(`${TOKEN_CACHE_PREFIX}:${account.id}`, { token, expiresAt });
    return `[${account.label}] 앱키=${mask(account.appkey)} → 토큰 발급 성공 (만료: ${new Date(expiresAt).toISOString()})`;
  } catch (e) {
    return `[${account.label}] 앱키=${mask(account.appkey)} → 토큰 발급 실패: ${e.message}`;
  }
}
