...
// 라벨(화면에 보일 계좌명)은 KIWOOM_APP_KEY_2_LABEL 처럼 선택 지정, 없으면 "계좌N"으로 표시.
export function getKiwoomAccounts(){
  ...
  if (process.env.KIWOOM_APP_KEY && process.env.KIWOOM_APP_SECRET) {
    accounts.push({
      ...
      label: process.env.KIWOOM_APP_KEY_LABEL || '계좌1',
    });
  }
  for (let n = 2; ; n++) {
    ...
    accounts.push({
      ...
      appkey,
      secretkey,
      label: process.env[`KIWOOM_APP_KEY_${n}_LABEL`] || `계좌${n}`,
    });
  }
  return accounts;
}
