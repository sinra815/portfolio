...
const TOKEN_CACHE_PREFIX = 'kiwoom:accessToken';
// 실제 만료 시각보다 여유를 두고 미리 재발급한다 — 요청 처리 도중 토큰이 만료되는 걸 방지.
const EXPIRY_BUFFER_MS = 5 * 60 * 1000;

// 키움 앱키는 1개당 계좌 1개에 매핑되는 구조라, 계좌를 여러 개 조회하려면 계좌 수만큼 앱키/
// 시크릿키 쌍이 필요하다. 기본 계좌는 기존과 같이 KIWOOM_APP_KEY/KIWOOM_APP_SECRET(하위
// 호환), 추가 계좌는 KIWOOM_APP_KEY_2/KIWOOM_APP_SECRET_2, _3 ... 처럼 번호를 붙여 등록한다.
// 라벨(화면에 보일 계좌명)은 KIWOOM_APP_KEY_2_LABEL 처럼 선택 지정, 없으면 NH와 같은 방식으로
// "키움계좌N"으로 표시.
export function getKiwoomAccounts(){
  const accounts = [];
  if (process.env.KIWOOM_APP_KEY && process.env.KIWOOM_APP_SECRET) {
    accounts.push({
      ...
      label: process.env.KIWOOM_APP_KEY_LABEL || '키움계좌1',
    });
  }
  for (let n = 2; ; n++) {
    const appkey = process.env[`KIWOOM_APP_KEY_${n}`];
    const secretkey = process.env[`KIWOOM_APP_SECRET_${n}`];
    if (!appkey || !secretkey) break;
    accounts.push({
      id: String(n),
      appkey,
      secretkey,
      label: process.env[`KIWOOM_APP_KEY_${n}_LABEL`] || `키움계좌${n}`,
    });
  }
  return accounts;
}
...
async function issueAccessToken(account){
  const res = await fetch(`${getBaseUrl()}/oauth2/token`, {
    ...
    body: JSON.stringify({ grant_type: 'client_credentials', appkey: account.appkey, secretkey: account.secretkey }),
  });
  const data = await res.json().catch(() => ({}));
  if (!res.ok || (data.return_code && data.return_code !== 0) || !data.token) {
    throw new Error(data.return_msg || `[${account.label}] 키움 접근토큰 발급에 실패했습니다.`);
  }
  return { token: data.token, expiresAt: parseKstExpiry(data.expires_dt) };
}

async function getAccessToken(redis, account, { forceRefresh } = {}){
  ...
  const { token, expiresAt } = await issueAccessToken(account);
...
// 자동으로 재시도한다.
export async function callKiwoom(redis, account, apiId, path, body, _retried){
  const token = await getAccessToken(redis, account, { forceRefresh: !!_retried });
  ...
  if (isAuthError && !_retried) {
    return callKiwoom(redis, account, apiId, path, body, true);
  }
  if (!res.ok || (data.return_code && data.return_code !== 0)) {
    throw new Error(data.return_msg || `[${account.label}] 키움 API 호출에 실패했습니다. (${apiId})`);
  }
  return data;
}
