...
- `api/kiwoom.js` — 키움증권 REST API 연동 (현재가 조회 + 계좌 잔고 조회를 한 파일에서 처리).
  Vercel Hobby 플랜의 서버리스 함수 개수 제한(배포당 12개) 때문에 기능별로 파일을 나누지 않고
  하나로 합쳤습니다.
  ...
  - `POST /api/kiwoom` (body: `{ id }`) — 실제 계좌 잔고 조회. 국내(`kt00018`)와 미국
    해외주식(`ust21070`) 잔고를 함께 조회해 하나의 목록으로 합쳐 돌려줍니다(해외 거래 계좌가
    없으면 해외 조회만 조용히 건너뜁니다). 키움 앱키가 특정 계좌 하나에 연결되는 구조라, `id`가
    `sinra815`가 아니면 거부합니다(다른 로그인 계정은 이 계좌를 조회할 수 없음).
