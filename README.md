...
로그인/비밀번호가 없는 개인 전용 단일 사용자 앱입니다.
화면은 프레임워크·빌드 도구 없는 순수 HTML/CSS/JS이고, 서버에 저장하거나 현재가를 불러오는
기능만 Vercel 서버리스 함수(`api/`)를 사용합니다.
...
Vercel 서버리스 함수입니다. 정적 파일만 올리면 "저장/불러오기"와 "금액 불러오기" 버튼이 동작하지 않습니다.

- `api/save.js` (`POST /api/save`) — 현재 상태를 Upstash Redis에 저장
- `api/load.js` (`GET /api/load`) — Upstash Redis에 저장된 상태를 반환
...
- **저장 / 불러오기 버튼**: 서버(Upstash Redis)에 저장·복원합니다. 다른 기기에서도 같은 데이터를
  볼 수 있습니다. 저장 키는 `rebalancer:data` 하나뿐이라, 저장할 때마다 이전 내용을 덮어씁니다.
...
인증이 전혀 없는 개인용 앱입니다. `/api/save`와 `/api/load`는 누구나 호출할 수 있고 저장 키도
하나뿐이므로, 배포 URL을 아는 사람은 데이터를 읽거나 덮어쓸 수 있습니다.
민감한 금액을 다루려면 Vercel Deployment Protection을 켜거나 API에 별도 인증을 추가해야 합니다.
